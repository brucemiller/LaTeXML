...
\begin{quote}
 \cmd{latexml --output=doca.xml doca}\\
 \cmd{latexml --output=docb.xml docb}\\
  \hbox{}\hspace{1in}\ldots\\
 \cmd{latexmlpost --prescan --dbfile=my.db --dest=doca.xhtml doca}\\
 \cmd{latexmlpost --prescan --dbfile=my.db --dest=docb.xhtml docb}\\
  \hbox{}\hspace{1in}\ldots\\
 \cmd{latexmlpost --noscan --dbfile=my.db --dest=doca.xhtml doca}\\
 \cmd{latexmlpost --noscan --dbfile=my.db --dest=docb.xhtml docb}\\
  \hbox{}\hspace{1in}\ldots
\end{quote}
...
\section{Basic \XML\ Conversion}\label{usage.conversion}\index{latexml!usage}
The command
\begin{quote}
 \cmd{latexml \textit{options} --output=\textit{doc}.xml \textit{doc}}
\end{quote}
...
\section{Basic Postprocessing}\label{usage.single}\index{latexmlpost!usage}
...
\section{Splitting the Output}\label{usage.multiple}\index{latexmlpost!usage!split pages}
...
\begin{quote}
 \cmd{--splitpath=\textit{xpath}}
\end{quote}
The default xpath is
\begin{quote}
 \cmd{//ltx:section | //ltx:bibliography | //ltx:appendix | //ltx:index}
\end{quote}
...
\section{Site processing}\label{usage.site}\index{latexmlpost!usage!site}
