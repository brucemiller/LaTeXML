...
\begin{description}
  ...
  \item[\cmd{--preload=\varfile{module}}] causes loading of \varfile[ltxml]{module}.
    For example, \cmd{--preload=LaTeX.pool} can be useful to force \LaTeX-mode
...
\begin{description}
  ...
  \item[\code{\cs{usepackage}\{\textit{package}\}}] (or related commands) loads
    \varfile[sty.ltxml]{package}.  Normally, \LaTeXML\ will not
    ...
    provide use cases.
  \item[\code{\cs{input}\{\varfile{file}\}}] loads the first
    \varfile[tex.ltxml]{file}, \varfile[tex]{file}, 
    \varfile[ltxml]{file} or \varfile{file} that is found.
\end{description}
...
\end{quote}
 can be used.  The \code{--css} option can be repeated to accumulate several stylesheets;
for example, the distribution provides several \code{navbar-left.css}, \code{navbar-right.css},
\code{theme-blue.css} and \code{amsart.css}, in addition to the \code{core.css} stylesheet
...
\item[Conversion] First, all \TeX\ sources must be converted
 ...
 \item[Scanning] Secondly, all \XML\ files must be split and scanned using
  the command
  \begin{quote}
   \cmd{latexmlpost --prescan --dbfile=\varfile{DB} --dest=\varfile[xhtml]{doci} \varfile{doci}}
  \end{quote}
 ...
 \item[Pagination] Finally, all \XML\ files are cross-referenced and converted into
   ...
   \begin{quote}
     \cmd{latexmlpost --noscan --dbfile=\varfile{DB} --dest=\varfile[xhtml]{doci} \varfile{doci}}
   \end{quote}
...
The \elementref{Math} element is a top-level container for any math mode material; it
...
\include{generated/latexml}
\include{generated/latexmlpost}

%%%======================================================================
\chapter[Modules]{Core Module Documentation}\label{coremodules}
\include{generated/LaTeXML}
\include{generated/LaTeXML_Object}
\include{generated/LaTeXML_Definition}
\include{generated/LaTeXML_Global}
\include{generated/LaTeXML_Error}
\include{generated/LaTeXML_Package}
\include{generated/LaTeXML_Parameters}
\include{generated/LaTeXML_State}

%\section{Digestion-related Modules}

\include{generated/LaTeXML_Token}
\include{generated/LaTeXML_Box}
\include{generated/LaTeXML_Number}
\include{generated/LaTeXML_Font}
\include{generated/LaTeXML_Mouth}
\include{generated/LaTeXML_Gullet}
\include{generated/LaTeXML_Stomach}

%\section{Construction-related Modules}
\include{generated/LaTeXML_Document}
\include{generated/LaTeXML_Model}
\include{generated/LaTeXML_Rewrite}

%\section{Math-related Modules}
\include{generated/LaTeXML_MathParser}

%%%======================================================================
\chapter[Utility Modules]{Utility Module Documentation}\label{utilitymodules}
\include{generated/LaTeXML_Util_Pathname}

%%%======================================================================
\chapter[Postprocessing Modules]{Postprocessing Module Documentation}\label{postmodules}
\include{generated/LaTeXML_Post}
...
{\small
\begin{description}
  \item[\pod{LaTeXML::Object}]: Abstract base class.
  \begin{description}
    % In Token.pm
    \item[\pod{LaTeXML::Token}]: A \TeX\ token: character/string with category code.
    \item[\pod{LaTeXML::Tokens}]: A list of \pod{LaTeXML::Token}s.
    \item[\pod{LaTeXML::Number}]: A \TeX\ number.
    \begin{description}
      \item[\pod{LaTeXML::Dimension}]: A \TeX\ dimension; number with unit.
      \begin{description}
        \item[\pod{LaTeXML::MuDimension}]: A \TeX\ math-mode dimension.
        \item[\pod{LaTeXML::Glue}]:  A \TeX\ dimension with shrink and stretch.
        \begin{description}
          \item[\pod{LaTeXML::MuGlue}]: A \TeX\ math-mode glue.
        \end{description}
      \end{description}
    \end{description}
    % In Box.pm
    \item[\pod{LaTeXML::Box}]: A digested character/string; base class for digested objects.
    \begin{description}
      \item[\pod{LaTeXML::MathBox}]: A digested character token in math.
      \item[\pod{LaTeXML::Comment}]: A digested comment.
      \item[\pod{LaTeXML::List}]: A list of text-mode boxes.
      \begin{description}
        \item[\pod{LaTeXML::MathList}]: A list of math-mode boxes.
      \end{description}
      \item[\pod{LaTeXML::Whatsit}]: A special digested object with arguments and properties; has its own
             rules for conversion into an document fragment.
    \end{description}
    % In Font.pm
    \item[\pod{LaTeXML::Font}]: A representation of a font, attached to digested objects.
    \begin{description}
      \item[\pod{LaTeXML::MathFont}]: A font in math; special rules for merging.
    \end{description}
    % In Node.pm
    \item[\pod{LaTeXML::Document}]: A representation of the document under construction.
    % In Definition.pm
    \item[\pod{LaTeXML::Definition}]: Represents the action of executable control sequences
    \begin{description}
      \item[\pod{LaTeXML::Expandable}]: A definition expandable in the Gullet, (eg.~a macro).
      \item[\pod{LaTeXML::Primitive}]: Definition for primitives, carried out in Stomach.
      \begin{description}
        \item[\pod{LaTeXML::Register}]: A definition for \TeX\ registers.
        \item[\pod{LaTeXML::Constructor}]: A definition that constructs document fragments;
             generates a Whatsit during digestion.
      \end{description}
    \end{description}
    % In Parameters.pm
    \item[\pod{LaTeXML::Parameters}]: A collection of \pod{LaTeXML::Parameter} representing a definition's
        parameter list.
    \item[\pod{LaTeXML::Parameter}]: A definition's parameter, including type, optional, etc.
%    \item[\LTXKeyVals]: Representation of \LaTeX-style Key-Value lists.
    % In Mouth.pm
    \item[\pod{LaTeXML::Mouth}]: The organ that converts characters (eg.~from files) into \pod{LaTeXML::Token}s.
    \begin{description}
      \item[\pod{LaTeXML::FileMouth}]: A mouth that tokenizes input from a file.
      \begin{description}      
        \item[\pod{LaTeXML::StyleMouth}]: A file mouth for reading style files.
      \end{description}
      \item[\pod{LaTeXML::PerlMouth}]: A fake mouth for reading perl modules.
    \end{description}
    % In Gullet.pm
    \item[\pod{LaTeXML::Gullet}]: The organ that expands \pod{LaTeXML::Token}s into other sequences of tokens;
      also provides common `parsing' operations such as reading numbers, delimited lists of tokens, etc.
    % In Stomach.pm
    \item[\pod{LaTeXML::Stomach}]: The organ that digests \pod{LaTeXML::Token}s, converting them to a tree of
       digested boxes; also contains the current state of grouping, bindings of definitions, etc.
    % In Model.pm
    \item[\pod{LaTeXML::Model}]: Represents the model of the document, extracted from the DTD.
  \end{description}
\end{description}
}
