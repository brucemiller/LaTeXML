...
text, Tokens, Boxes, WhatchamaCallIts and \XML, on the one hand,
...
\index{WhatchamaCallIt(LaTeXML::)@{\ttfamily  WhatchamaCallIt(LaTeXML::)}!architecture}%
...
into \ltxpod{Core::List}s of \ltxpod{Core::Box}es and \ltxpod{Core::WhatchamaCallIt}s
...
reading and processing its arguments and wrapping these up in a \ltxpod{Core::WhatchamaCallIt}.
...
as augmenting the \ltxpod{Core::WhatchamaCallIt} with additional properties.
...
Given the \ltxpod{Core::List} of \ltxpod{Core::Box}es and \ltxpod{Core::WhatchamaCallIt}s,
...
A \ltxpod{Core::WhatchamaCallIt} is absorbed by invoking the associated \ltxpod{Core::Definition::Constructor}
...
text, tokens, boxes and WhatchamaCallIts, as well as macros and macro expansion,
primitives and digestion, and finally WhatchamaCallIts and constructors.
...
of digested \ltxpod{Core::Box}es or \ltxpod{Core::WhatchamaCallIt}s.
...
digests \ltxcode|$tokens| (a \ltxpod{Core::Tokens}), returning a list of \ltxpod{Core::Box}es and \ltxpod{Core::WhatchamaCallIt}s.
...
receiving the arguments and properties from the \ltxpod{Core::WhatchamaCallIt};
...
\item \ltxcode|#1|,\ltxcode|#2|,\ldots \ltxcode|#%name%|
   ...
   or named property of the WhatchamaCallIt;
...
\item \ltxcode.properties=> {%hash} | CODE($stomach,$arg1,...).
  provides a set of properties to store in the \ltxpod{Core::WhatchamaCallIt} for eventual use
  in the constructor \ltxcode|$replacement|.  If a subroutine is used,
  it also should return a hash of properties;
\item \ltxcode|beforeDigest=>CODE($stomach)|,\\
      \ltxcode|afterDigest=>CODE($stomach,$WhatchamaCallIt)|
  ...
  or to augment the properties of the \ltxpod{Core::WhatchamaCallIt} (after);
\item  \ltxcode|beforeConstruct=>CODE($document,$WhatchamaCallIt)|,\\
      \ltxcode|afterConstruct=>CODE($document,$WhatchamaCallIt)|
...
\item \ltxcode&reversion=>$string|CODE(...)&,
   \ltxcode|alias=>$cs|
  can be used when the \ltxpod{Core::WhatchamaCallIt} needs to be reverted into \TeX\ code,
...
\item \ltxcode|ToString($stuff)| converts \ltxcode|$stuff| to a string,
  ...
  and attribute values.  Note that if \ltxcode|$stuff| contains WhatchamaCallIts
...
\begin{itemize}
\item \ltxcode|afterDigestBegin=>CODE($stomach,$WhatchamaCallIt)|
...
\ltxpod{Core::Box} (or \ltxpod{Core::WhatchamaCallIt}) responsible for its creation;
...
\input{pods/LaTeXML_Core_WhatchamaCallIt}
% Various objects
