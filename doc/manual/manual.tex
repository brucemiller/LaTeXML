...
--svg %or% --nosvg
\end{lstlisting}
Image conversion requires one of the Perl modules \code{Graphics::Magick} or
\code{Image::Magick}; by default, the first one that can be loaded is used.
To force the use of a specific one (e.g.~if one of them fails on your system),
set the environment variable \code{LATEXML\_IMAGECLASS}:
\begin{lstlisting}[style=shell]
LATEXML_IMAGECLASS=Image::Magick latexmlpost --dest=doc.html doc.xml
\end{lstlisting}
