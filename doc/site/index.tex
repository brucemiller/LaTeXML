...
\begin{lstlisting}[style=shell]
   perl -MCPAN -e shell
   cpan> install XML::LibXML, XML::LibXSLT, Parse::RecDescent, File::Which,
   cpan> install URI, IO::String, libwww, JSON::XS, Archive::Zip, Image::Size
   cpan> quit
\end{lstlisting}
...
\item[ImageMagick] provides a handy library of image manipulation routines.
...
yum install perl-XML-LibXML perl-XML-LibXSLT \
    perl-Parse-RecDescent perl-File-Which \
    perl-URI perl-IO-string perl-libwww-perl \
    perl-JSON-XS perl-Archive-Zip \
    ImageMagick ImageMagick-perl perl-Image-Size
\end{lstlisting}
...
\begin{lstlisting}[style=shell]
   sudo apt-get install   \
      ...
      imagemagick perlmagick libimage-size-perl
\end{lstlisting}
Then continue by using the general procedure from \ref{build.source}.
...
\begin{lstlisting}[style=shell]
  sudo port install    \
      ...
      p5-parse-recdescent p5-file-which \
      p5-uri p5-io-string p5-libwww-perl \
      p5-json-xs p5-archive-zip \
      p5-perlmagick p5-image-size
\end{lstlisting}
Then continue by using the general procedure from \ref{build.source}.
...
or \ref{install.windows.git} after \ref{install.windows.prerequisites}.
