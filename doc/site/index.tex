...
\item If \texttt{libxml2} and \texttt{libxslt} are are not already installed:\\
  ...
  download and install \texttt{libxml2} and \texttt{libxslt}.
...
\begin{lstlisting}[style=shell]
   perl -MCPAN -e shell
   cpan> install Parse::RecDescent, XML::LibXML, XML::LibXSLT, DB_File
   cpan> quit
\end{lstlisting}
\item If \texttt{ImageMagick} and its Perl binding are not already installed:\\
  ...
  to download and install ImageMagick being sure to enable
...
\end{lstlisting}
Afterwards, you may use the general procedure to
...
\paragraph{Installing prerequisites}
LaTeXML runs under \href{http://strawberryperl.com}{Strawberry Perl}.
You may install it using either the released source or SVN repository
after installing the prerequisites as follows:
\begin{itemize}
\item Follow the instructions at 
  \href{http://strawberryperl.com}{Strawberry Perl}
   to download and install the perl environment\\
...
\end{lstlisting}
\item Follow the instructions at
   \href{http://sourceforge.net/projects/ghostscript/}{Ghostscript}
   to download and Install the Ghostscript program
   (needed by Image::Magick)
\item Follow the instructions at
   \href{http://www.imagemagick.org/script/binary-releases.php}{Image Magick}
   to download and install the ImageMagick library binary (or from source, if preferred)
\item Install Image::Magick using the command
\begin{lstlisting}[style=shell]
  cpan -i Image::Magick
\end{lstlisting}
...
Installation using the released source follows the general procedure,
but using the \texttt{dmake} command provided by Strawberry Perl:
\begin{itemize}
\item Download \CurrentTarball\\
...
\paragraph{Installing from SVN}
You may install a very current version from SVN
\begin{itemize}
\item If you don't already have an SVN client,
   ...
   \item Fetch \LaTeXML\ from the repository:
\item Checkout a fresh copy of \LaTeXML:
\begin{lstlisting}[style=shell]
 svn co https://svn.mathweb.org/repos/LaTeXML
 cd LaTeXML
 perl Makefile.PL
