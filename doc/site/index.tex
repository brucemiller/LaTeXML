...
See \ref{install} for installation instructions, including OS-specific notes.

\paragraph{Current Release:}\label{download.current}
\CurrentRelease.
See also the \href{Changes}{Change Log}.

\paragraph{SVN Repository}\label{download.svn}
...
\paragraph{Licence}\label{license}
...
\paragraph{Archived Releases:}\label{archive}
\AllReleases.

%============================================================
\section{Installation}\label{install}
\LaTeXML\ itself is written in `pure' Perl, but requires several
perl modules.  A recent \href{http://www.perl.org/}{Perl}
with sufficient Unicode support is required: version 5.8 or later.
An installation of \TeX\ is also needed.


There are several options for installation.
You may install \LaTeXML\ \ref{install.source}
(whether using the tarball or checkout from \ref{download.svn})
or, for some operating systems, there may be prepared binaries.
Likewise, you may install the prerequisies from their sources,
CPAN or use the facilities of your operating system.
Check \ref{install.osnotes} for your operating system
for notes that may simplify the installation.

\subsection{Prerequisites from CPAN}
...
\begin{alltt}
   yum --nogpgcheck localinstall \CurrentRPM
\end{alltt}
...
% \begin{alltt}
%    yum --nogpgcheck install perl-Parse-RecDescent  \textbackslash\\
...
\href{MacPorts}{http://www.macports.org}.
To install them, use
\begin{alltt}
