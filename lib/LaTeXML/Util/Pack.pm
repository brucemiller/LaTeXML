...
sub detect_source {
    ...
    while (<$README_FH>) {
      ...
      if ($directive) {
        if ($directive eq 'toplevelfile') {
          ...
          pathname_unlink($ignored_filepath) if pathname_e $ignored_filepath; } } }
  ...
  foreach my $tex_file (@TeX_file_members) {
    ...
    next unless pathname_e $tex_file;    # skip deleted "ignored" files.
...
# Special heuristic 1: If we have multiple "best" candidates, prefer those closer to the archive root '/'
    ...
    if (scalar(@files_by_likelihood) > 1) {
      my @with_bbl = grep { my $base = $_; $base =~ s/\.tex$//; pathname_e "$base.bbl"; } @files_by_likelihood;
