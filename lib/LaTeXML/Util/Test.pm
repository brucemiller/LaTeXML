...
sub latexml_tests {
  ...
  if (!opendir($DIR, $directory)) {
    ...
    SKIP: {
        ...
        SKIP: {
            skip("No file $test.xml", 1) unless (pathname_f "$test.xml");
        ...
        SKIP: {
            skip("No file $test.xml and/or $test-post.xml", 1)
              unless ((pathname_f "$test.xml") && (pathname_f "$test-post.xml"));
            next unless check_requirements($test, 1, $$requires{'*'}, $$requires{$name});
...
####    ['timeout',            10],
  ...
  if (!$generate) {
    ...
    pathname_unlink "$base.test.xml" if pathname_e "$base.test.xml";
  }
  else {
    #TODO: Skip 3 tests
    ...
    system(@invocation);
    pathname_chdir($current_dir);
    move("$base.test.xml", "$base.xml") if pathname_e "$base.test.xml";
  }
  return; }
...
sub texlive_version {
  ...
  if (my $path = $ENV{LATEXML_KPSEWHICH}) {
    if (($path =~ s/kpsewhich/tex/i) && (pathname_X $path)) {
      $tex = $path; } }
