...
sub csymbol_with_fallback {
  my ($item, $fallback_cd, $fallback_meaning) = @_;
  my ($cd, $meaning);
  if ($meaning = (ref $item) && $item->getAttribute('meaning')) {
    $cd = $item->getAttribute('omcd') || 'latexml'; }
  return ['m:csymbol', { cd => $cd || $fallback_cd }, $meaning || $fallback_meaning]; }

DefMathML('Token:SUPERSCRIPTOP:?', undef, sub {
    return csymbol_with_fallback($_[0], 'ambiguous', 'superscript'); });
DefMathML('Token:SUBSCRIPTOP:?', undef, sub {
    return csymbol_with_fallback($_[0], 'ambiguous', 'subscript'); });
