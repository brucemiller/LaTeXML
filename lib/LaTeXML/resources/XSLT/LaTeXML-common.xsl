...
<xsl:stylesheet
  ...
  <!-- Very awkward to express in XSLT 1.0 but,
      while we have an xpath-indexed ltx:break child (at index n),
      count all figure|table|float|graphics elements that have
      as many (= n-1) ltx:break preceding siblings, as
      the current (n'th) ltx:break. 
      
      Recursively pass around that max count, updating it at each next break.
      And of course keep in mind the edge cases before any breaks, and after the last.
  -->
  <xsl:template name="count_cells_at_break">
    <xsl:param name="break_iter_index"/>
    <xsl:param name="previous_count"/>
    <xsl:variable name="this_count">
      <xsl:value-of select="count(*[(self::ltx:figure | self::ltx:table | self::ltx:float | self::ltx:graphics) and (count(preceding-sibling::ltx:break) = $break_iter_index)])"/>
    </xsl:variable>
    
    <xsl:variable name="new_max">
      <xsl:choose>
        <xsl:when test="$previous_count &lt; $this_count">
          <xsl:value-of select="$this_count"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="$previous_count"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <!-- <xsl:message>
        At iter <xsl:value-of select="$break_iter_index"/>: last max was <xsl:value-of select="$previous_count"/> , new max is <xsl:value-of select="$new_max"/>.
    </xsl:message> -->
    <xsl:choose>
      <xsl:when test="ltx:break[$break_iter_index+1]">
        <xsl:call-template name="count_cells_at_break">
          <xsl:with-param name="previous_count" select="$new_max"/>          
          <xsl:with-param name="break_iter_index" select="1 + $break_iter_index"/>
        </xsl:call-template>
      </xsl:when>
      <!-- at the last break, a final resursive call is in order,
           counting all pieces upto the end of the parent. -->
      <xsl:when test="ltx:break[$break_iter_index]">
        <xsl:call-template name="count_cells_at_break">
          <xsl:with-param name="previous_count" select="$new_max"/>         
          <xsl:with-param name="break_iter_index" select="1 + $break_iter_index"/>
        </xsl:call-template>
      </xsl:when>
      <!-- at the very end, return the largest count we saw during the walk -->
      <xsl:otherwise>
