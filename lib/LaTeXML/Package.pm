...
sub FindFile_aux {
  ...
  if (pathname_is_absolute($file)) {    # And if we've got an absolute path,
    if (!$options{noltxml}) {
      return $file . '.ltxml' if pathname_f ($file . '.ltxml'); }    # No need to search, just check if it exists.
    return $file if pathname_f $file;    # No need to search, just check if it exists.
  ...
  if (my $result = (!$options{searchpaths_only}) && pathname_kpsewhich(@candidates)) {
    return (pathname_f $result ? $result : undef); }
