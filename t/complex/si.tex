...
Table \ref{tab:si_base_units}
\newcommand{\showunit}[1]{#1 & \texttt{\Backslash#1} & \unit{\csname#1\endcsname}}
\begin{table}
\caption{SI base units}\label{tab:si_base_units}
\centering
...
Table \ref{tab:coherent_derived}
\begin{table}
\caption{Coherent derived units}\label{tab:coherent_derived}
\centering
...
Table \ref{tab:non_si_units}
\begin{table}
\caption{Non-SI units}\label{tab:non_si_units}
\centering
\begin{tabular}{lll}\toprule
Unit & Macro & Symbol \\\midrule
...
%Deprecated in siunitx v3, but make sure we still have them:
%\unit{\clight}, \unit{\electronmass}, \unit{\bohr}, \unit{\hartree}
...
Table \ref{tab:si_prefixes}
\begin{table}
\caption{SI prefixes}\label{tab:si_prefixes}
\centering
...
Table 5 %\ref{tab:abbrevs}
% we have trouble referring to a longtable?

%\begin{table}
%\centering
\begin{longtable}{lcc}\toprule
\caption{Abbreviated units\label{tab:abbrevs}}\\
Unit & Abbreviation & Symbol \endfirsthead
\caption{Abbreviated units (cont)}\\
Unit & Abbreviation & Symbol \\\midrule\endhead
...
Table \ref{tab:binpref}
\begin{table}
\caption{Binary prefixes}\label{tab:binpref}
\newcounter{tablerow}
% the documentation numbers are hardcoded in
% /usr/local/texlive/2024/texmf-dist/source/latex/siunitx/siunitx.tex
...
\clearpage
% a whole lot of floating tables just finished
...
Table \ref{tab:align:exp}
\begin{table}
\centering
\caption{The \texttt{table-align-exponent} option%
\label{tab:align:exp}}\sisetup{table-format = 1.3e2}
...
\begin{table}
\caption{The \texttt{drop-exponent} option.%
\label{tab:exp:omit}}
\centering
\begin{tabular}{@{}
